...
export default function SageDetail() {
  ...
  const seoTitle = sage ? `Biography of ${sage.name}` : "Sage Biography";
  const seoDesc = sage?.description ||
    "Explore the life and teachings of a spiritual sage on The Nirvanist.";

  const sageBreadcrumb = sage ? createBreadcrumbSchema([
    { name: "Home", url: "https://www.thenirvanist.com" },
    { name: "Sages", url: "https://www.thenirvanist.com/sages" },
    { name: sage.name }
  ]) : null;

  const personSchema = sage ? createPersonSchema({
    name: sage.name,
    description: sage.description?.slice(0, 160),
    image: sage.image,
